// package authclient provides support to accerss the auth service
package authclient
...
// New constructs an Auth that can be used to talk to the auth sercie
func New(url string, logFunc Logger, options ...func(cln *Client)) *Client {
	cln := Client{
		url:     url,
		logFunc: logFunc,
		http:    &defaultClient,
...
// WIthClient adds a custom client fo processing requests. It's recomment
...
func (cln *Client) rawRequest(ctx context.Context, method string, url string, headers map[string]string, r io.Reader, v any) error {
	...
	for key, value := range headers {
		cln.logFunc(ctx, "authclient: rawRequest", "key", key, "value", value)
	...
	cln.logFunc(ctx, "authclient: rawRequest", "statuscode", resp.StatusCode)
	...
	case http.StatusOK:
		if err := json.Unmarshal(data, v); err != nil {
			return fmt.Errorf("failed: reponse: %s, decoding error: %w", string(data), err)
		}
		return nil
