...
var ErrForbidden = errors.New("attemptted action is not allowed")

// Claims represents the authorixation claims transmitted via a JWT
type Claims struct {
...
type KeyLookup interface {
	...
	PubliceKey(kid string) (key string, err error)
}

// COnfig trpresents information required to initialize auth
type Config struct {
	Log       *logger.Logger
	KeyLookup KeyLookup
	Issuer    string
...
// Auth is use to authenticate clients. It can generate a token for a
// set of user claims and receate the claims by parsing the token
type Auth struct {
	keyLookup KeyLookup
...
func (a *Auth) Authenticate(ctx context.Context, bearerToken string) (Claims, error) {
	...
	pem, err := a.keyLookup.PubliceKey(kid)
	if err != nil {
	...
	if err := a.opaPolicyEvaluation(ctx, regoAuthentication, RuleAuthenticate, input); err != nil {
		return Claims{}, fmt.Errorf("authentication dfailed: %w", err)
	}
